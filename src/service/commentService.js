...
class CommentService {
    async createComment(commentData) {
        try{
            ...
            if (parent_comment) {
                await Post.findByIdAndUpdate(parent_comment, { $inc: { replies_count: 1 } });
            }

            return await this.getCommentById(Comment._id, author);
        }   
        catch(error){
    ...
    async updateComment(commentId, updateData, requestingUserId) {
        try{
            ...
            const forrbiddenFields = ['post', 'author', 'parent_comment', 'replies_count', 'likes_count', 'is_deleted'];
            for (const field of forrbiddenFields) {
